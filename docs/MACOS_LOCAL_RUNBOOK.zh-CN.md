...
- **左侧 −／百分比／＋**：调整角色大小，范围 70%~160%，每次 10%。改的是
  `character_size` 设置，跟 Settings → Appearance 里那个滑块是同一个值，会立即持久化。
- **中间横条**：按住拖动可移动整个角色窗口（走系统窗口拖拽）。
- **齿轮按钮**：打开 Settings。
- **折角按钮**：隐藏角色，之后从托盘菜单 Show Persona 恢复。
- **四个角**：按住拖动直接缩放角色——往外拉变大，往里推变小，跟着指针连续变化。
  松手才写入设置。

两种改大小的方式对应不同场景：拖角落适合凭感觉调，−／＋ 适合要精确值（每次 10%）。
两者改的是同一个 `character_size`，范围都是 70%~160%。

窗口本身的尺寸不在边框上——直接拖窗口边缘即可。

边框本身是**纯虚线轮廓、不带填充**，不会给角色蒙上一层色；按钮各自带深色底保证
可读性。
